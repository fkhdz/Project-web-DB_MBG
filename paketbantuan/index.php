...
<head>
    ...
    <title>Kelola Paket Bantuan - MBG Workspace</title>
    ...
    <aside class="flex flex-col w-64 bg-white border-r border-slate-100 p-4 shrink-0 hidden lg:flex shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10">
        <div class="flex flex-col h-full">
            ...
            <nav class="flex flex-col gap-1.5 flex-grow">
                ...
                <a class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-primary-50 text-primary-600 transition-colors" href="../paketbantuan/index.php">
                    <span class="material-symbols-outlined text-[22px]" style="font-variation-settings: 'FILL' 1;">inventory_2</span>
                    <p class="text-sm font-semibold">Paket Bantuan</p>
                </a>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto">
        ...
        <div class="flex-1 p-6 lg:p-10 max-w-7xl mx-auto w-full">
            ...
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-slate-600 text-sm font-medium">Stok Paket</span>
            </div>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 class="text-3xl font-bold text-slate-900 tracking-tight">Katalog Paket Bantuan</h2>
                    <p class="text-slate-500 mt-1.5 text-sm">Kelola spesifikasi, ketersediaan, dan komposisi item bantuan.</p>
                </div>
                <div class="flex flex-wrap gap-2 sm:gap-3">
                    <a href="../item/index.php" class="inline-flex items-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm">
                ...
                <div class="mb-6">
                    <?php if ($_GET['msg'] == 'deleted') : ?>
                        <div class="flex items-center gap-3 p-4 bg-emerald-50 text-emerald-700 rounded-xl border border-emerald-100/50 shadow-sm" role="alert">
                            ...
                            <div class="text-sm font-semibold">Berhasil! Data paket berhasil dihapus.</div>
                        </div>
                    <?php elseif ($_GET['msg'] == 'error') : ?>
                        <div class="flex items-center gap-3 p-4 bg-red-50 text-red-700 rounded-xl border border-red-100/50 shadow-sm" role="alert">
                            ...
                            <div class="text-sm font-semibold">Gagal! Data tidak dapat dihapus (mungkin masih terhubung dengan tabel lain).</div>
                        </div>
                    <?php elseif ($_GET['msg'] == 'updated') : ?>
                        <div class="flex items-center gap-3 p-4 bg-primary-50 text-primary-700 rounded-xl border border-primary-100/50 shadow-sm" role="alert">
                            ...
                            <div class="text-sm font-semibold">Berhasil! Data paket berhasil diperbarui.</div>
                        </div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div class="relative w-full max-w-md">
                        ...
                        <input type="text" class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-all placeholder:text-slate-400 shadow-sm" placeholder="Cari nama atau jenis paket..."/>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Identitas Paket</th>
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Stok (Qty)</th>
                        ...
                        if (mysqli_num_rows($query) > 0) {
                            while ($row = mysqli_fetch_assoc($query)) {
                                ...
                                $badge_class = "bg-slate-50 text-slate-600 ring-1 ring-inset ring-slate-500/20"; // Default
                                
                                if (strpos($jenis, 'makanan') !== false || strpos($jenis, 'sembako') !== false) {
                                    $badge_class = "bg-blue-50 text-blue-600 ring-1 ring-inset ring-blue-500/20";
                                    ...
                                    $badge_class = "bg-purple-50 text-purple-600 ring-1 ring-inset ring-purple-500/20";
                                }
                        ?>
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 max-w-[250px]">
                                    ...
                                    <p class="text-xs text-slate-500 mt-1 line-clamp-2" title="<?= htmlspecialchars($row['deskripsi']) ?>"><?= htmlspecialchars($row['deskripsi']) ?></p>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide <?= $badge_class ?>">
                                        <?= htmlspecialchars($row['jenis_bantuan']) ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-1 text-xs text-slate-600">
                                        <div class="flex items-center gap-1.5"><span class="material-symbols-outlined text-[14px] text-slate-400">scale</span> <?= htmlspecialchars($row['berat_total']) ?></div>
                                        ...
                                        <div class="flex items-center gap-1.5"><span class="material-symbols-outlined text-[14px] text-slate-400">event_busy</span> Exp: <?= htmlspecialchars($row['kadaluarsa']) ?></div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-block px-3 py-1 bg-slate-100 text-slate-800 text-sm font-bold rounded-lg border border-slate-200">
                                        <?= htmlspecialchars($row['kuantitas']) ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <a href="komposisi.php?id=<?= $row['paket_id'] ?>" class="flex items-center justify-center size-8 rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-all shadow-sm" title="Atur Komposisi/Isi Paket">
                                            <span class="material-symbols-outlined text-[18px]">view_list</span>
                                        </a>
                                        <div class="w-px h-4 bg-slate-200 mx-1"></div>
                                        <a href="update.php?id=<?= $row['paket_id'] ?>" class="flex items-center justify-center size-8 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-primary-600 hover:border-primary-200 hover:bg-primary-50 transition-all shadow-sm" title="Edit">
                                            <span class="material-symbols-outlined text-[18px]">edit</span>
                                        </a>
                                        <a href="delete.php?id=<?= $row['paket_id'] ?>" onclick="return confirm('Hapus data paket ini?')" class="flex items-center justify-center size-8 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all shadow-sm" title="Hapus">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-12">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-slate-500 text-sm font-medium">Belum ada data paket bantuan di sistem.</p>
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>
                ...
                <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <p class="text-xs text-slate-500 font-medium">Menampilkan data paket dari database</p>
                </div>
